...
@Repository
public class GlosarioJDBCRepositoryImpl implements IGlosarioRepository {
	...
	public void create(Glosario r) {
		try {
			...
			throw new BadRequestException("El registro con id=" + r.getGlosId() + " ya existe");
		}
	...
	public List<Glosario> showAll() {
		StringBuilder sql = new StringBuilder();
		sql.append(env.getProperty("sql.glosario"));
		return jdbc.query(sql.toString(), new GlosarioRowMapper());
	}
