...
@RestController
@RequestMapping(value = "/estado/api")
@CrossOrigin
@Api(tags = "Estado - Rest Controller", value = "Servicio para la controlar los estados de las PQRSFD")
public class EstadoController {

	@Autowired
	private IEstadoService estadoService;

	public static final String URL_CONTROLLER = "/estado/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todos los estados que se manejan en las pqrsdf", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Estado.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", estadoService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear un estado de pqrsdf.", value = "Crear estado", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Estado creado con exito"),
			@ApiResponse(code = 401, message = "El estado no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Estado estado) {
		try {
			estadoService.create(estado);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Estado creado con éxito", null), HttpStatus.OK);
