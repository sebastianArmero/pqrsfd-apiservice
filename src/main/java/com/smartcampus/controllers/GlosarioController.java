...
@RestController
@RequestMapping(value = "/glosario/api")
@CrossOrigin
@Api(tags = "Glosario - Rest Controller", value = "Servicio para la controlar el glosario de las PQRSFD")
public class GlosarioController {

	@Autowired
	private IGlosarioService glosarioService;

	public static final String URL_CONTROLLER = "/glosario/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todo el glosario que se manejan en las pqrsdf", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Glosario.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", glosarioService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear el glosario de pqrsdf.", value = "Crear glosario", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Glosario creado con exito"),
			@ApiResponse(code = 401, message = "El glosario no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Glosario glosario) {
		try {
			glosarioService.create(glosario);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Glosario creado con éxito", null), HttpStatus.OK);

		} catch (NotFoundException e) {
			return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.NOT_FOUND.value(),
					"No es posible crear el glosario",
