...
@RestController
@RequestMapping(value = "/preguntas/api")
@CrossOrigin
@Api(tags = "Preguntas - Rest Controller", value = "Servicio para la controlar las preguntas de las calificaciones de las PQRSFD")
public class PreguntasController {

	@Autowired
	private IPreguntasService preguntasService;

	public static final String URL_CONTROLLER = "/preguntas/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todos las preguntas que se manejan en las pqrsdf", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Preguntas.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", preguntasService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear una pregunta para calificar pqrsdf.", value = "Crear pregunta", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "pregunta creado con exito"),
			@ApiResponse(code = 401, message = "El asunto no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Preguntas preguntas) {
		try {
			preguntasService.create(preguntas);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "pregunta creada con éxito", null), HttpStatus.OK);
