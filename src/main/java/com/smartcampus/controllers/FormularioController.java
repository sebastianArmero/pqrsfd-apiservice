...
@RestController
@RequestMapping(value = "/formulario/api")
@CrossOrigin
@Api(tags = "Formulario - Rest Controller", value = "Servicio para la controlar los formmulario de las encuestas")
public class FormularioController {

	@Autowired
	private IFormularioService formularioService;

	public static final String URL_CONTROLLER = "/formulario/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todos los formulario de la encuesta", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Asunto.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", formularioService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear un formulario.", value = "Crear formulario", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "formulario creado con exito"),
			@ApiResponse(code = 401, message = "El formulario no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Formulario formulario) {
		try {
			formularioService.create(formulario);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Formulario creado con éxito", null), HttpStatus.OK);

		} catch (NotFoundException e) {
			return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.NOT_FOUND.value(),
					"No es posible crear el formulario",
