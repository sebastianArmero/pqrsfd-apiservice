...
@RestController
@RequestMapping(value = "/parametros/api")
@CrossOrigin
@Api(tags = "Parametros - Rest Controller", value = "Servicio para los parametrosde los dias  de las PQRSFD")
public class ParametrosController {

	@Autowired
	private IParametrosService parametrosService;

	public static final String URL_CONTROLLER = "/parametros/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todos los parametros que se manejan en las pqrsdf", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Parametros.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", parametrosService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear un parametro de pqrsdf.", value = "Crear parametro", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Parametro creado con exito"),
			@ApiResponse(code = 401, message = "El parametro no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Parametros parametro) {
		try {
			parametrosService.create(parametro);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Parametro creado con éxito", null), HttpStatus.OK);
