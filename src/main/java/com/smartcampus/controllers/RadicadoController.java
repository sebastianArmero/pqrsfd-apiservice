...
@RestController
@RequestMapping(value = "/radicado/api")
@CrossOrigin
@Api(tags = "Radicado - Rest Controller", value = "Servicio para la controlar los asuntos de las PQRSFD")
public class RadicadoController {

	@Autowired
	private IRadicadoService radicadoService;

	public static final String URL_CONTROLLER = "/radicado/api";

	@ApiOperation(nickname = "showAll", notes = "Este método obtiene todos el radicado que se manejan en las pqrsdf", value = "Ver todos los objetos creados", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Radicado.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", radicadoService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear un radicado de pqrsdf.", value = "Crear radicado", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Radicado creado con exito"),
			@ApiResponse(code = 401, message = "El radicado no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Radicado radicado) {
		try {
			radicadoService.create(radicado);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Radicado creado con éxito", null), HttpStatus.OK);

		} catch (NotFoundException e) {
			return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.NOT_FOUND.value(),
					"No es posible crear el radicado",
