...
@RestController
@RequestMapping(value = "/encuesta/api")
@CrossOrigin
@Api(tags = "Encuesta - Rest Controller", value = "Servicio para la controlar las encuestas de satisfacción de las PQRSFD")
public class EncuestaController {

	@Autowired
	private IEncuestaService encuestaService;

	public static final String URL_CONTROLLER = "/encuesta/api";
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Encuesta.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", encuestaService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear una encuesta de pqrsdf.", value = "Crear encuesta", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Encuesta creada con exito"),
			@ApiResponse(code = 401, message = "El asunto no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Encuesta encuesta) {
		try {
			encuestaService.create(encuesta);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Encuesta creada con éxito", null), HttpStatus.OK);

		} catch (NotFoundException e) {
			return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.NOT_FOUND.value(),
					"No es posible crear la encuesta",
