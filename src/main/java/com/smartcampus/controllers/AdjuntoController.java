...
@RestController
@RequestMapping(value = "/adjunto/api")
@CrossOrigin
@Api(tags = "Adjunto - Rest Controller", value = "Servicio para la controlar los adjuntos de las PQRSFD")
public class AdjuntoController {

	@Autowired
	private IAdjuntoService adjuntoService;

	public static final String URL_CONTROLLER = "/adjunto/api";
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Valor de un objeto obtenido con éxito", response = Adjunto.class),
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", adjuntoService.showAll()),
				HttpStatus.OK);
	}

	@ApiOperation(nickname = "create", notes = "Este método permite crear un archivo adjunto de pqrsdf.", value = "Crear asunto", response = SimpleObjectResponse.class, produces = "application/json")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Adjunto creado con exito"),
			@ApiResponse(code = 401, message = "El asunto no se pudo registrar", response = SimpleObjectMessage.class),
			@ApiResponse(code = 404, message = "No se encontró el recurso solicitado", response = SimpleObjectMessage.class),
			@ApiResponse(code = 500, message = "Error con la conexión a la base de datos", response = SimpleObjectMessage.class) })
	@PostMapping(value = "/create")
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody List<Adjunto> adjunto) {
		try {
			adjuntoService.createAdjunto(adjunto);
			return new ResponseEntity<>(
					new SimpleObjectResponse(HttpStatus.OK.value(), "Adjunto creado con éxito", null), HttpStatus.OK);

		} catch (NotFoundException e) {
			return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.NOT_FOUND.value(),
					"No es posible crear el adjunto",
	...
	public ResponseEntity<SimpleObjectResponse> showByIdRequ(@PathVariable Long id) {
		return new ResponseEntity<>(new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito",
				adjuntoService.showByIdRequ(id)), HttpStatus.OK);
	}
