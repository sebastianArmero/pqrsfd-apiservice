...
@RestController
@RequestMapping(value = "/asunto/api")
@CrossOrigin
@Api(tags = "Asunto - Rest Controller", value = "Servicio para la controlar los asuntos de las PQRSFD")
public class AsuntoController {

	@Autowired
	private IAsuntoService asuntoService;
	...
	public ResponseEntity<SimpleObjectResponse> showAll() {
		return new ResponseEntity<>(
				new SimpleObjectResponse(HttpStatus.OK.value(), "Objetos obtenidos con éxito", asuntoService.showAll()),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<SimpleObjectResponse> create(@RequestBody Asunto asunto) {
		try {
			asuntoService.create(asunto);
			return new ResponseEntity<>(
